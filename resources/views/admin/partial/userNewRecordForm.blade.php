<!--
user new record form
- displays a form to create a new user record
-->
...
<x-ui.card>
    ...
    <x-slot:header>
        <div class="flex gap-2">
            <x-ui.header h="3">Create new user</x-ui.header>
        </div>
    </x-slot:header>

    <x-slot:content>

        <x-ui.form method="POST" action="{{ route('admin.action.store_record', ['table' => 'users']) }}">
            ...
            <x-ui.form-group>
                ...
                <x-ui.input id="f_email" name="email" type="email" placeholder="email" value="{{ old('email') }}"
