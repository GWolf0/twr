...
<x-ui.card>
    ...
    <x-slot:header>
        <div class="flex gap-2">
            <x-ui.header h="3">Edit vehicle</x-ui.header>
        </div>
    </x-slot:header>
    ...
    <x-slot:content>
        ...
        <x-ui.form method="PATCH"
            action="{{ route('admin.action.update_record', ['table' => 'vehicles', 'id' => $record->id]) }}">
            ...
            <x-ui.form-group>
                ...
                <x-ui.input id="f_price_per_hour" name="price_per_hour" type="number" step="0.01"
                    placeholder="price per hour" value="{{ old('price_per_hour', $record->price_per_hour) }}"
                    required />
            </x-ui.form-group>

            @php
                $availabilityValue = old('availability', $record->availability);
            @endphp
            <x-ui.form-group>
            ...
            <x-forms.vehicle-images-field :media="old('media', $record->media)" />
